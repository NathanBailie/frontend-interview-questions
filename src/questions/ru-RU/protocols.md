...
<details>
<summary><span>30. В чем <b>отличия TCP и UDP</b>?</span></summary>
<br />

| Атрибут                        | TCP                                                   | UDP                                            |
| ------------------------------ | ----------------------------------------------------- | ---------------------------------------------- |
| **Модель**                     | Ориентирован на соединение                            | Без установления соединения                    |
| **Надёжность**                 | Гарантированная доставка, подтверждения, ретрансляция | Нет гарантий доставки                          |
| **Порядок**                    | Сохраняет порядок сегментов                           | Порядок не гарантируется                       |
| **Контроль перегрузки/потока** | Есть (flow и congestion control)                      | Отсутствует                                    |
| **Накладные расходы**          | Высокие: установка соединения, подтверждения          | Низкие: короткие заголовки                     |
| **Типичные сценарии**          | Веб, файлы, почта, базы данных — важна целостность    | VoIP, видео, игры, DNS — важна низкая задержка |

---

Короткое объяснение

- TCP устанавливает соединение, нумерует сегменты, подтверждает приём и повторно отправляет потерянные данные; предпочтителен для транзакций и передачи файлов.
- UDP отправляет отдельные датаграммы без подтверждений и порядка; ответственность за надёжность ложится на приложение; предпочтителен для real‑time и мультимедиа.
...
<details>
<summary><span>31. Зачем нужны <b>тайм-ауты</b> в HTTP-запросах?</span></summary>
<br />

Тайм-ауты ограничивают время ожидания ответа, чтобы избежать зависания, освободить ресурсы и вовремя обработать ошибку. Они повышают стабильность приложения и улучшают пользовательский опыт.
...
<!--
В чем разница между SSE, Long Polling и WebSoket?
