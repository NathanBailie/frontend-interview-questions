...
**Пессимистичные блокировки** — предполагают высокую вероятность конфликтов, поэтому ресурсы блокируются заранее.

- **Shared Lock (S-lock)** — разрешает чтение, запрещает изменение.
- **Exclusive Lock (X-lock)** — запрещает и чтение, и изменение.
- **Уровни блокировки:** строки, страницы, таблицы.

**Оптимистичные блокировки** — предполагают низкую вероятность конфликтов, проверка происходит при фиксации.

- **MVCC** — каждая транзакция работает со своей версией данных.
- **Таймстемпы** — сравнение по времени начала транзакций.
- **Хеши/контрольные суммы** — проверка изменений перед фиксацией.

**Итог:**  
Пессимистичные — надёжнее, но медленнее.  
Оптимистичные — быстрее, но требуют проверок.
...
<details>
<summary><span>29. Какие есть <b>аномалии</b>?</span></summary>
<br />

- **Грязное чтение (Dirty Read)** — транзакция читает данные, которые были изменены другой незавершённой транзакцией.
- **Неповторяющееся чтение (Non-repeatable Read)** — при повторном чтении тех же данных в рамках одной транзакции получаются разные значения, потому что другая транзакция их изменила.
- **Фантомное чтение (Phantom Read)** — при повторном выполнении запроса появляются новые строки, добавленные другой транзакцией.
- **Потерянное обновление (Lost Update)** — два процесса читают одно и то же значение и обновляют его, но одно из обновлений теряется.
- **Неупорядоченное чтение (Unordered Read)** — чтение данных происходит в непредсказуемом порядке из-за отсутствия согласованности между транзакциями.
...
Чем отличается синхронная репликация от асинхронной? Риски.
Алгоритмы: Round Robin vs Least Connections.
Паттерны для распределенных транзакций: Transactional Outbox/Inbox
Что такое self join
Что такое подготовленные парамеры в TypeORM
 -->
